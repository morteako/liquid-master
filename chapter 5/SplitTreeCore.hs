...
splitTree ap e mes p i (Deep v pr Empty sf )
    | p vpr = 
        let 
            DSplit l x r = splitDigit ap mes p i pr
        in  FSplit (toTree ap e mes l) x (deepL ap e mes r Empty sf)
    | otherwise = 
        let DSplit l x r = splitDigit ap mes p vpr sf
        in  FSplit (deepR ap e mes pr Empty l) x (toTree ap e mes r)
    where 
        vpr = i `ap` measureList ap e mes pr
splitTree ap e mes p i (Deep v pr m sf )
    ...
    | p vm = 
        let 
            ...
            DSplit l x r = splitDigit ap mes p (vpr `ap` measureFT ap e (measureNode ap e mes) ml) (nodeToList xs)
        in 
            FSplit (deepR ap e mes pr ml l) x (deepL ap e mes r mr sf)
    | otherwise = 
        let DSplit l x r = splitDigit ap mes p vm sf
...
deep ap e mes pr m sf = 
    Deep 
        (measureList ap e mes pr `ap` measureFT ap e (measureNode ap e mes) m `ap` measureList ap e mes sf)
        pr 
        m 
        sf
