...
{-@ type Monoid v = (ap:v -> v -> v,e:v,assoc: (x:v) -> (y:v) -> (z:v) -> {ap x (ap y z) == ap (ap x y) z},identity: (x:v) -> { ap x e == x && ap e x == x}) @-}
{-@ type MonoidVer v a = (ap:v -> v -> v,e:v, mes:a->v, assoc: (x:v) -> (y:v) -> (z:v) -> {ap x (ap y z) == ap (ap x y) z},identity: (x:v) -> { ap x e == x && ap e x == x}) @-}
type Mon v a = (v -> v -> v, v, a -> v ) 
type MonVer v a = (v -> v -> v, v, a -> v , v->v->v->(),v -> ()) 
...
{-@ reflect viewL2 @-}
{-@ viewL2 :: Monoid v a -> ft:FingerTree v a -> ViewL (FingerTree v) a / [ftDigsize ft, 0]@-}
viewL2 :: Mon v a -> FingerTree v a -> ViewL (FingerTree v) a
viewL2 mon Empty                = NilL
viewL2 mon (Single x)           = ConsL x Empty
viewL2 mon (Deep v (p:pr) m sf) = ConsL p (deepL2 mon pr m sf) 

{-@ reflect deepL2 @-}
{-@ 
deepL2 ::  Monoid v a -> {v:[a] | len v <= 4 } -> m:FingerTree v (Node v a) -> Dig a -> FingerTree v a / [ftDigsize m, 1]
@-}
deepL2 :: Mon v a -> [a] -> FingerTree v (Node v a) -> Digit a -> FingerTree v a
deepL2 mon@(ap,e,mes) [] m sf = case viewL2 (ap, e, measureNode2 mon) m of
    NilL -> toTree2 mon sf
    ConsL a m' -> deep2 mon (nodeToList a) m' sf
deepL2 mon (p:pr) m sf = deep2 mon (p:pr) m sf
...
toTree2 ::
    ...
    -> FingerTree v a 
@-}
toTree2 :: Mon v a -> [a] -> FingerTree v a
-- -> {vv:[a] | len vv <= 4}      NOT SURE
toTree2 mon [] = Empty --NOT SURE
toTree2 mon [a] = Single a
toTree2 mon (a:as) = un mon a (toTree2 mon as)
...
measureNode2 :: 
    ...
    ->  v 
@-}
measureNode2 :: Mon v a -> Node v a -> v
measureNode2 _ (Node2 v _ _) = v
measureNode2 _ (Node3 v _ _ _) = v
...
@-}
deep2 :: 
   Mon v a -> [a] -> FingerTree v (Node v a) -> [a] -> FingerTree v a
deep2 mon@(ap,e,_) pr m sf = 
    Deep 
        (un mon pr `ap` measureFT ap e (measureNode2 mon) m `ap` un mon sf)
        pr 
        m 
        sf
